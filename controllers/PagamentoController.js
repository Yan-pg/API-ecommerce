...
class PagamentoController {
    ...
    async show(req, res, next){
        try{
            const pagamento = await PagamentoController.findOne({ _is: req.params.id, loja: req.query.loja})
            ...
            const sutuacao = (pagamento.pagSeguroCode) ? await getTransactionStatus(pagamento.pagSeguroCode) : null
    ...
    async pagar(req, res, next){
        ...
        try {
            ...
            return res.send({ pagametno, payload })
        }catch(e){ 
            next(e)
        }
    }
    ...
    async update(req, res, next){
        ...
        try{
            const pagamento = await PagamentoController.findOne({ _is: req.params.id, loja})
            if(!pagamento) return res.status(400).send({ error: "Pagamento nao existe"})
            ...
            await registroPedido.save()
            // Enviar email de aviso para o cliente - aviso atualizacao de pagamento
    ...
    async verNotificacao(req, res, next){
        try {
            ...
            const pagamento = await Pagamento.findById({ pagSeguroCode: result.code })
            if(!pagamento) return res.status(400).send({ error: "Pagamento nao existe"})
